...
export default function InventoryTab({ grns, materials, outwards, currentUser, onSaveGrn, theme = 'dark' }: InventoryTabProps) {
  ...
  const [inventoryFilterTab, setInventoryFilterTab] = useState<'All' | 'Warehouse' | 'LowStock' | 'NearExpiry'>('All');
  const showLowStockOnly = inventoryFilterTab === 'LowStock';
  const showNearExpiryOnly = inventoryFilterTab === 'NearExpiry';
  const subTab = 'Warehouse';
  const showWarehouse = true;
  const showJobwork = false;
  ...
  const [sortColumn, setSortColumn] = useState<SortColumn>('arNo');
  ...
  const matchedMaterials = searchTerm.trim()
    ? materials.filter(m =>
        ...
        (m.palletNo && m.palletNo.toLowerCase().includes(searchTerm.toLowerCase())) ||
        (m.drumNo && m.drumNo.toLowerCase().includes(searchTerm.toLowerCase()))
      )
    : [];
  ...
  const moveColumn = (key: keyof VisibleColumns, direction: 'up' | 'down') => {
    setColumnOrder(prev => {
      ...
      const originalCodeIndex = prev.indexOf('code' as any);
      const finOrder: (keyof VisibleColumns)[] = [];
      let customCounter = 0;
      for (let i = 0; i < prev.length; i++) {
        if (i === originalCodeIndex) {
          finOrder.push('code' as any);
        } else {
    ...
    switch (key) {
      ...
      case 'arNo':
        return (
          <th 
            key="arNo"
            onClick={() => handleSort('arNo')} 
            className="px-4 py-3.5 text-left cursor-pointer hover:bg-white/5 transition whitespace-nowrap"
          >
            <span>AR Number</span> {renderSortIndicator('arNo')}
          </th>
        );
      case 'batch':
        return (
  ...
  const renderBodyCell = (key: keyof VisibleColumns, row: any) => {
    ...
    switch (key) {
      ...
      case 'arNo':
        return (
          <td key="arNo" className={`px-4 py-3 font-mono text-[11px] font-extrabold text-left ${
      ...
      case 'releaseDate':
        return (
          <td key="releaseDate" className={`px-4 py-3 text-center font-mono whitespace-nowrap ${
            ...
            {row.qcReleaseDate ? row.qcReleaseDate.split('T')[0].split('-').reverse().join('-') : (row.grnDate ? row.grnDate.split('-').reverse().join('-') : 'N/A')}
          </td>
        );
      default:
        return null;
  ...
  const filteredRows = computedRows.filter(row => {
    ...
    const matchesSearch = 
      ...
      (row.material && row.material.palletNo && row.material.palletNo.toLowerCase().includes(searchTerm.toLowerCase())) ||
      (row.material && row.material.drumNo && row.material.drumNo.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className={`text-xs mt-1 font-sans ${theme === 'light' ? 'text-slate-500' : 'text-[#87929a]'}`}>
            Unified inventory master directory. Track QC approved warehouse stocks.
          </p>
        </div>

        {/* Button: Modify Columns Display Checklist */}
      ...
      {searchTerm.trim() !== '' && matchedMaterials.length > 0 && (
        <div className="bg-white border border-stone-250/90 rounded-2xl p-6 shadow-sm space-y-6 my-4 animate-fade-in">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-stone-100 pb-4">
            <div>
              <div className="text-[10px] uppercase font-bold tracking-widest text-blue-600">Live Master Audit Inquiry</div>
              <h3 className="text-base font-bold font-sans text-slate-850 flex items-center gap-2">
                <ClipboardCheck className="w-5 h-5 text-blue-600" />
                <span>Transaction Movement Report for matching items ({matchedMaterials.length})</span>
              </h3>
            </div>
            <span className="text-[10px] font-mono bg-blue-50 text-blue-600 px-2.5 py-1 rounded-full border border-blue-100">
              Query: "{searchTerm}"
            </span>
          </div>
          ...
          <div className="space-y-6">
            {matchedMaterials.slice(0, 3).map(mat => {
              ...
              return (
                <div key={mat.id} className="bg-[#FAF9F5]/70 border border-stone-200/60 rounded-xl p-4.5 space-y-4">
                  {/* Item header */}
                  <div className="flex flex-wrap items-center justify-between gap-3 border-b border-stone-200/40 pb-2.5">
                    <div>
                      <div className="text-xs font-bold text-slate-800">{mat.materialName}</div>
                      <div className="text-[10px] font-mono text-stone-500 mt-0.5">Code: {mat.materialCode} | Category: {mat.category} | UOM: {mat.uom}</div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div className="bg-emerald-50 border border-emerald-100/80 rounded-xl p-3">
                      <div className="text-[9px] uppercase font-bold text-emerald-800 tracking-wider">Total Inward Received</div>
                      <div className="text-base font-black text-emerald-700 mt-1">{totalInwardQty.toLocaleString()} <span className="text-xs font-semibold">{mat.uom}</span></div>
                      <div className="text-[9px] text-stone-500 mt-1.5 leading-tight truncate" title={receivedLocations.join(', ')}>
                        Locations: {receivedLocations.length > 0 ? receivedLocations.join(', ') : 'None'}
                      </div>
                    </div>

                    <div className="bg-blue-50 border border-blue-100/80 rounded-xl p-3">
                      <div className="text-[9px] uppercase font-bold text-blue-800 tracking-wider font-sans">Total Issued Outward</div>
                      <div className="text-base font-black text-blue-700 mt-1">{totalOutwardQty.toLocaleString()} <span className="text-xs font-semibold">{mat.uom}</span></div>
                      <div className="text-[9px] text-stone-500 mt-1.5 leading-tight truncate" title={issuedDepartments.join(', ')}>
                        Issued To: {issuedDepartments.length > 0 ? issuedDepartments.join(', ') : 'None'}
                      </div>
                    </div>

                    <div className="bg-amber-50 border border-amber-140/80 rounded-xl p-3">
                      <div className="text-[9px] uppercase font-bold text-amber-800 tracking-wider font-sans">Live Net Balance</div>
                      <div className="text-base font-black text-amber-700 mt-1">{currentAvailable.toLocaleString()} <span className="text-xs font-semibold">{mat.uom}</span></div>
                      <div className="text-[9px] text-stone-500 mt-1.5 font-sans">
                  ...
                  <div className="space-y-2">
                    <div className="text-[10px] uppercase font-bold text-stone-500 tracking-wider font-sans">Movement Timeline (Ledger)</div>
                    {ledgerEvents.length === 0 ? (
                      <div className="text-center py-4 bg-white rounded-xl text-[11px] text-stone-400 font-sans border border-stone-200/50">
                        No transactions found for this material in ledger logs.
                      </div>
                    ) : (
                      <div className="overflow-x-auto rounded-xl border border-stone-200/70 bg-white">
                        <table className="min-w-full divide-y divide-zinc-100 text-[11px]">
                          <thead className="bg-[#FAF9F5] text-stone-605 uppercase text-[9px] tracking-wider font-sans border-b border-stone-200/40">
                            <tr>
                          ...
                          <tbody className="divide-y divide-zinc-100 text-slate-700 bg-white">
                            {ledgerEvents.map(evt => (
                              <tr key={evt.id} className="hover:bg-slate-50 transition duration-150">
                                ...
                                <td className="px-3 py-1.5 text-center whitespace-nowrap">
                                  <span className={`inline-block px-2 py-0.5 rounded text-[8px] font-black ${
                                    evt.type === 'INWARD' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-blue-50 text-blue-700 border border-blue-200'
                                  }`}>
                                    {evt.type}
                                  </span>
                                </td>
                                <td className="px-3 py-1.5 whitespace-nowrap font-mono">{evt.batchNo}</td>
                                <td className="px-3 py-1.5 text-right whitespace-nowrap font-bold text-slate-900">
