...
// Remove any undefined values and nested undefined values before saving to Firestore, preventing setDoc errors
...
const SEED_MATERIALS: Material[] = [
  { id: 'mat1', materialCode: 'RM-MCC-101', materialName: 'Microcrystalline Cellulose (MCC)', category: 'Raw Material', uom: 'KG', minStock: 2000, createdAt: new Date().toISOString() },
  { id: 'mat2', materialCode: 'RM-SSG-201', materialName: 'Sodium Starch Glycolate (SSG)', category: 'Raw Material', uom: 'KG', minStock: 800, createdAt: new Date().toISOString() },
  { id: 'mat3', materialCode: 'PPM-PVC-01', materialName: 'Primary Packaging PVC Film', category: 'PPM', uom: 'KG', minStock: 500, createdAt: new Date().toISOString() }
];
...
const SEED_GRNS: Grn[] = [
  {
    ...
    grnNo: 'WH/IN/00002',
    grnDate: '2026-06-19',
    supplierName: 'SUREN HEALTH CARE',
    materialId: 'mat1',
    materialName: 'Microcrystalline Cellulose (MCC)',
    batchNo: 'MCC-9921',
    qty: 1500,
    mfgDate: '2026-05-15',
    expDate: '2029-05-14',
    invoiceNo: 'INV/2026/0882',
    warehouseLocation: 'RM Store',
    receivedBy: 'SUMIT SURELIYA',
    qcStatus: 'Pending',
    sourceType: 'Supplier',
    createdOn: '2026-06-19T14:08:00.000Z'
  },
  {
    id: 'grn_seed2',
    grnNo: 'WH/IN/00003',
    ...
    supplierName: 'SUREN HEALTH CARE',
    materialId: 'mat2',
    materialName: 'Sodium Starch Glycolate (SSG)',
    batchNo: 'SSG-8812',
    qty: 500,
    mfgDate: '2026-05-10',
    expDate: '2029-05-09',
    ...
    qcStatus: 'Approved',
    sourceType: 'Supplier',
    createdOn: '2026-06-19T14:10:00.000Z',
    qcReleaseDate: '2026-06-20'
  }
];
...
const SEED_OUTWARDS: Outward[] = [
  {
    id: 'out_seed1',
    outwardNo: 'WH/OUT/00001',
    outwardDate: '2026-06-20',
    materialId: 'mat2',
    materialName: 'Sodium Starch Glycolate (SSG)',
    batchNo: 'SSG-8812',
    qty: 150,
    department: 'Production Dispensing',
    issuedBy: 'SUMIT SURELIYA',
    remarks: 'Dispensed to Production Line A for tablet compression',
...
export const dbService = {
  ...
  async saveUser(user: User): Promise<void> {
    if (isFirebaseActive && firestoreDb) {
      const path = 'users';
      try {
        await setDoc(doc(firestoreDb, path, user.id), cleanPayload(user));
      } catch (error) {
  ...
  async saveMaterial(material: Material): Promise<void> {
    if (isFirebaseActive && firestoreDb) {
      const path = 'materials';
      try {
        await setDoc(doc(firestoreDb, path, material.id), cleanPayload(material));
      } catch (error) {
  ...
  async saveGrn(grn: Grn): Promise<void> {
    if (isFirebaseActive && firestoreDb) {
      const path = 'grn';
      try {
        await setDoc(doc(firestoreDb, path, grn.id), cleanPayload(grn));
      } catch (error) {
  ...
  async saveOutwardRecord(outward: Outward): Promise<void> {
    if (isFirebaseActive && firestoreDb) {
      const path = 'outwards';
      try {
        await setDoc(doc(firestoreDb, path, outward.id), cleanPayload(outward));
      } catch (error) {
  ...
  async saveSupplier(supplier: Supplier): Promise<void> {
    if (isFirebaseActive && firestoreDb) {
      const path = 'suppliers';
      try {
        await setDoc(doc(firestoreDb, path, supplier.id), cleanPayload(supplier));
      } catch (error) {
  ...
  async fetchCompanyProfile(): Promise<CompanyProfile> {
    ...
    if (isFirebaseActive && firestoreDb) {
      ...
      try {
        ...
        if (!profile) {
          await setDoc(docRef, cleanPayload(defaultProfile));
  ...
  async saveCompanyProfile(profile: CompanyProfile): Promise<void> {
    ...
    if (isFirebaseActive && firestoreDb) {
      const path = 'company_profile';
      try {
        await setDoc(doc(firestoreDb, path, 'main'), cleanPayload(profile));
      } catch (error) {
